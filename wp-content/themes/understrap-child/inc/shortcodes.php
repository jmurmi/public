...
// Link moore
function link_moore( $atts ) {

	// Attributes
	extract( shortcode_atts(
		array(
			'texto' => 'Diseño web: moore.net',
			'url'	=> 'https://moore.net',
		), $atts )
	);
...
add_shortcode( 'link_moore', 'link_moore' );
...
function paginas_hijas() {
	...
	if ( is_post_type_hierarchical( $post->post_type ) /*&& '' == $post->post_content */) {
		...
		if ($query->have_posts() ) {
			...
			// $r .= '<h3>'.__( 'Contenido en', 'moore' ).' "'.$post->post_title.'"</h3>';
			// $r .= '<ul>';
...
function get_redes_sociales() {
    ...
    foreach ($redes_sociales as $red => $fa_class) {
    	...
    	if( '' != $url) {
	    	$r .= '<a href="'.$url.'" target="_blank" rel="nofollow" title="'.sprintf( __( 'Abrir %s en otra pestaña', 'moore' ), $red ).'"><span class="red-social '.$red.' fa fa-'.$fa_class.'"></span></a>';
    	}
    }

    // $r .= '<span class="follow-us">' . __( 'Follow us', 'moore' ) . '</span>';
...
function zarabici_vc_separator() {
	return '<div style="height:40px" aria-hidden="true" class="wp-block-spacer"></div>';
}
add_shortcode( 'vc_separator', 'zarabici_vc_separator' );

function zarabici_latest_post($atts) {
	extract( shortcode_atts(
		array(
				'category' 	=> false,
		), $atts)	
...
add_shortcode( 'latest_post', 'zarabici_latest_post' );

function zarabici_vc_gallery($atts) {
	extract( shortcode_atts(
		array(
				'images' 	=> false,
				'columns'	=> 4,
...
add_shortcode( 'vc_gallery', 'zarabici_vc_gallery' );
